...
- \curl -L https:// get.rvm.io | bash -s stable (without the space after 'https://')
...
The MySQL database password and the secret key used for signing session cookies are not stored in source control and must be supplied in the file '.env'.  The MySQL password is the same password provided above in the 'grant' MySQL command.  The secret key is generated using rake.  To set the database password and the secret key execute: 
...
Phusion Passenger Standalone web server is used in production.  To install Passenger execute:
- gem install passenger

NGINX reverse proxy serveris used in production.  To install NGINX (and it's dependencies) execute:
...
- TODO: ADD SERVER BLOCKS

TODO: NGINX Startup/Stop/Reload scripts
