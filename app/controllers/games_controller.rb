class GamesController < ApplicationController
  ...
  def create
    ...
    unless @game.errors.any?
      ...
      return redirect_to(play_path, flash: {success: 'New game created successfully.'})
    else
  ...
  def join
    # Confirm game exists
    return redirect_to(play_path, {flash: {error: 'Unknown game.'}}) unless @game = Game.find_by(id: params[:game_id])
    # Attempt to add player to game
    @game.add_player(current_user)
    return @game.errors.any? ? redirect_to(play_path, {flash: {error: @game.errors.full_messages.to_sentence}}) : redirect_to(game_path(@game))
  end

  # Show a game
  def show
    # Confirm game exists
    return redirect_to(play_path, {flash: {error: 'Unknown game.'}}) unless @game = Game.find_by(id: params[:id])
    # Confirm user is playing game or game is complete
    return redirect_to(play_path, {flash: {error: 'You are not playing that game.'}}) unless @game.players.include?(current_user) || @game.complete?
    @round = @game.round
  end
